...
function createSigmoidGateModel(numFeatures, hiddenUnits) {
    // Исправленная архитектура: используем custom layer для умножения
    const model = tf.sequential();
    
    // Входной слой
    const input = tf.input({shape: [numFeatures]});
    
    // Создаем модель функциональным API
    const modelInput = tf.input({shape: [numFeatures]});
    ...
    // Применяем гейт
    const gateOutput = gateLayer.apply(modelInput);
    
    // Умножение: input * gate (эмулируем multiply)
    const multiplyLayer = tf.layers.multiply();
    const gatedFeatures = multiplyLayer.apply([modelInput, gateOutput]);
    ...
    // Создаем модель
    const functionalModel = tf.model({
        inputs: modelInput,
        outputs: finalOutput
    });
...
function displayModelSummary() {
    ...
    if (AppState.useSigmoidGate) {
        summaryHTML += `
            <div style="margin-bottom: 10px; padding: 12px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #9b59b6;">
                <div style="font-weight: bold; color: #2c3e50; margin-bottom: 4px;">
                    <i class="fas fa-filter"></i> Sigmoid Gate Layer
                </div>
                <div style="font-size: 0.9rem; color: #7f8c8d;">
                    <div>Units: ${AppState.processedData.featureNames.length}</div>
                    <div>Activation: sigmoid</div>
                    <div>Purpose: Learns feature importance weights</div>
                </div>
            </div>
            
            <div style="margin-bottom: 10px; padding: 12px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #3498db;">
                <div style="font-weight: bold; color: #2c3e50; margin-bottom: 4px;">
                    <i class="fas fa-times-circle"></i> Feature Gating (Multiply)
                </div>
                <div style="font-size: 0.9rem; color: #7f8c8d;">
                    <div>Operation: input × sigmoid_gate</div>
                    <div>Purpose: Applies learned importance to features</div>
                </div>
            </div>
        `;
    }
    ...
    AppState.model.layers.forEach((layer, index) => {
        ...
        // Пропускаем если это функциональная модель
        if (layerType === 'InputLayer' || layer.name === 'sigmoid_gate' || layer.name === 'multiply') {
            return;
        }
